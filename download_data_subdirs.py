...
def _file_name(row):
    h = "0x%08X" % (zlib.crc32(row['url'].encode('utf-8')) & 0xffffffff)
    h = h[2:]
    dirname = "%s/%s/%s/%s" % (h[0], h[1], h[2], h[3])
    if not os.path.isdir("%s/%s" % (row['folder'], dirname)):
        os.makedirs("%s/%s" % (row['folder'], dirname))
    return "%s/%s/%s_%s" % (row['folder'], dirname, row.name, h)
